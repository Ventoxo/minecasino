...
green
'''
bet_win_data_red = ['carrot' for _ in range(15)]
bet_win_data_black = ['potato' for _ in range(15)]
bet_win_data_zero = ['cactus' for _ in range(1)]
bet_win_data = bet_win_data_zero + bet_win_data_red + bet_win_data_black


def pay(player_name, bet_sum, coef=1.8):
    pyautogui.press('t')
...
def hey():
    ...
    change_lang()
    pyautogui.press('t')
    pyautogui.write(f"!Yf cthdtht hf,jnftn jykfqy-rfpbyj/  Regbnt yf ferwbjyt rfhnjire? vjhrjdm bkb rfrnec/   ",
                    interval=0.02)
    pyautogui.write(
        f"Rfrnec - l;trgjn (cnfdrf*8)/ vjhrjdm b rfhnjirf (cnfdrf*1/8)? yj b ifyc dsquhfnm ,jkmit/ Hbcreq b gj,t;lfq!",
        interval=0.02)
    pyautogui.press('enter')
    change_lang()
